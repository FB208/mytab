...
(function () {
  ...
  // 动态导入共享模块
  let webdavSyncModule = null;
  async function getWebdavSyncModule() {
    if (!webdavSyncModule) {
      webdavSyncModule = await import('./webdav-sync.js');
    }
    return webdavSyncModule;
  }
  ...
  async function readAllFromShim() {
    ...
    const DEFAULT_DATA = { folders: [], backgroundImage: '', lastModified: new Date('2020-01-01').getTime() };
    const DEFAULT_SETTINGS = { webdav: { url: '', username: '', password: '' }, backup: { enabled: true, frequencyHours: 4, maxSnapshots: 100 }, client: { identifier: '' }, locale: { mode: 'auto' }, theme: {} };
  ...
  async function doBackup(source) {
    ...
    const syncModule = await getWebdavSyncModule();
    
    await syncModule.doBackupToCloud({
      data,
      settings,
      source,
      createClient: getWebDavClient
    });
  }
  ...
  async function checkCloudDataInWeb() {
    ...
    return syncModule.checkCloudData({
      localData,
      settings,
      createClient: getWebDavClient
    });
  ...
  async function syncFromCloudInWeb(fileName) {
    ...
    return syncModule.syncFromCloudData({
      fileName,
      settings,
      createClient: getWebDavClient,
      doBackup,
